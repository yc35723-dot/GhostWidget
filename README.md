...
**GhostWidget** 是一款專為 Android 設計的「隱形」桌面小工具 。旨在為想要在桌面上想放載具條碼，但又不想擋到桌布的人們設計，只在需要時出現! 它在預設情況下完全透明，僅在您單擊或雙擊時才會顯示預設的圖片（如載具條碼、電子票券或其他您喜歡的圖片）。
...
- **完全透明 (Ghost Mode)**：平時完全隱藏於桌面背景，不破壞您的精心排版。
- **單/雙擊雙圖設定**：支援為「單擊」與「雙擊」分別設定不同的圖片，靈活度高。
- **極致響應速度**：經過優化的點擊判定邏輯，支援自定義 **150ms - 1000ms** 的反應延遲。
- **螢幕常亮支援**：開啟功能後，只要圖片正在顯示，螢幕就不會自動關閉，方便掃描條碼。
- **Material You 動態取色**：介面主題會自動跟隨系統桌布顏色進行變化（支援 Android 12+）。
...
3. **調整反應速度**：在 App 內使用滑桿調整最適合您的點擊節奏（建議設定為 200ms）。
4. **操作小工具**：
   - **單擊**：顯示第一張圖片 / 再次單擊隱藏。
   - **雙擊**：顯示第二張圖片 / 再次雙擊隱藏。
...
- **核心技術**：
  ...
  - `Material 3` & `Dynamic Colors` 提供現代感介面。
